...
(function () {
  ...
  <div class="ei-overlay" id="eiOverlay" role="dialog" aria-modal="true">
    <div class="ei-modal">
      ...
      <div class="ei-footer">
        <div class="ei-popup-warn" id="eiPopupWarn" style="display:none; background:#fef3c7; border:1px solid #fde68a; color:#92400e; border-radius:10px; padding:10px 12px; font-size:12px; margin-bottom:10px; line-height:1.5;">
          ⚠ <strong>팝업이 차단되었어요.</strong> 브라우저 주소창의 차단 아이콘에서 MaroPass 팝업을 허용한 뒤 다시 눌러주세요.
        </div>
        <button class="ei-cta" id="eiCta">↗ 새 창에서 응시하기</button>
        <div class="ei-cta-hint">응시 화면이 <strong>새 창</strong>에서 열립니다. 팝업 차단을 해제해 주세요.</div>
  ...
  function startExam() {
    if (!state.nextUrl) { close(); return; }
    // 정책 10 — 응시는 새 창(터치클래스 사용자 web)으로 열린다.
    const win = window.open(state.nextUrl, 'kopass_exam', 'noopener=no,noreferrer=no');
    const warn = document.getElementById('eiPopupWarn');
    if (!win || win.closed || typeof win.closed === 'undefined') {
      // 팝업 차단 감지 — 모달 유지하고 안내 노출
